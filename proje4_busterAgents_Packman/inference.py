...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key in self.keys():
            _keys.append(key)
            probs.append(self[key])

        return random.choices(_keys, probs)[0]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        noisyDistance_prob = busters.getObservationProbability(noisyDistance, true_distance)

        return noisyDistance_prob
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in ghostPositions:
            noisyDistanceProb = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            self.beliefs[ghostPosition] *= noisyDistanceProb
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        distributions = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(distributions)
        n = self.numParticles
        size = len(distributions)
        while n > size:
            self.particles += distributions
            n -= size
        self.particles += distributions[:n]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                list *= self.getObservationProb(noisy_dist, pacmanPosition, p[i], self.getJailPosition(i))
            new_dist[p] += list
